...
namespace KostumKita.Model // Tambahkan namespace yang sesuai
{
    public class KeranjangContext // Ubah dari internal ke public
    {
        ...
        public void HapusItemDariKeranjang(string namaKostum, string jenisKostum, string statusTransaksi = "sewa")
        {
            using (var conn = new NpgsqlConnection(connectionString))
            {
                try
                {
                    ...
                    if (jenisKostum.ToLower() == "tradisional")
                    {
                        deleteQuery = "DELETE FROM carts WHERE id_kostum_tradisional = @id_kostum AND UPPER(jenis_kostum) = @status";
                    }
                    else
                    {
                        deleteQuery = "DELETE FROM carts WHERE id_kostum_entertainment = @id_kostum AND UPPER(jenis_kostum) = @status";
                    }
